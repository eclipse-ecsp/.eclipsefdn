...
orgs.newOrg('automotive.ecsp', 'eclipse-ecsp') {
  ...
  _repositories+:: [
    ...
    orgs.newRepo('transformers') {
      ...
      secrets: [
        orgs.newRepoSecret('SONAR_TOKEN') {
          value: "pass:bots/automotive.ecsp/sonarcloud.io/token-tranformers",
        },
      ],
    },
   ...
   orgs.newRepo('uidam-portal') {
    ...
    orgs.newRepo('token-validator') {
      allow_merge_commit: true,
      allow_update_branch: false,
      delete_branch_on_merge: false,
      description: "Java library for validating JWT tokens and managing public keys",
      web_commit_signoff_required: false,
      workflows+: {
        default_workflow_permissions: "write",
      },
      secrets: [
        orgs.newRepoSecret('SONAR_TOKEN') {
          value: "pass:bots/automotive.ecsp/sonarcloud.io/token-token-validator",
        },
      ],
    },
  ],
